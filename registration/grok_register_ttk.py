...
_DATA_DIR = os.path.abspath(os.environ.get("REGISTRATION_DATA_DIR") or _REG_DIR)
CONFIG_FILE = os.path.abspath(os.environ.get("REGISTRATION_CONFIG_FILE") or os.path.join(_DATA_DIR, "config.json"))
...
_EMAILS_USED_FILE = os.path.join(_DATA_DIR, "emails_used.txt")
_EMAILS_ERROR_FILE = os.path.join(_DATA_DIR, "emails_error.txt")
...
_COOKIE_DIR = os.path.join(_DATA_DIR, "cookies", "grok")
...
def _add_token_to_grok2api_pools_sync(raw_token, email="", log_callback=None):
    ...
    remote_disabled = os.environ.get("REGISTRATION_DISABLE_REMOTE_IMPORT", "").strip().lower() in {"1", "true", "yes"}
    if config.get("grok2api_auto_add_remote", False) and not remote_disabled:
        try:
...
def get_tempmail_lol_base():
    return str(config.get("tempmail_lol_api_base", TEMPMAIL_LOL_API_BASE) or TEMPMAIL_LOL_API_BASE).rstrip("/")


def tempmail_lol_create_inbox():
    payload = {}
    prefix = str(config.get("tempmail_lol_prefix", "") or "").strip()
    domain = str(config.get("tempmail_lol_domain", "") or "").strip()
    if prefix:
        payload["prefix"] = prefix
    if domain:
        payload["domain"] = domain
    resp = http_post(
        f"{get_tempmail_lol_base()}/inbox/create",
        json=payload,
        headers={"Content-Type": "application/json"},
    )
    resp.raise_for_status()
    ...
    if data.get("captcha_required"):
        raise Exception("TempMail.lol requires a Turnstile token for inbox creation")
    address = str(data.get("address") or "").strip()
    token = str(data.get("token") or "").strip()
    if not address or not token:
        raise Exception("TempMail.lol inbox response did not contain address and token")
    return address, token


def tempmail_lol_get_messages(token):
    resp = http_get(
        f"{get_tempmail_lol_base()}/inbox",
        params={"token": token},
    )
    resp.raise_for_status()
    data = resp.json()
...
def tempmail_lol_message_text(message):
    parts = []
...
def tempmail_lol_get_oai_code(
    ...
    while time.time() < deadline:
        ...
        if resend_callback and time.time() >= next_resend_at:
            try:
                if resend_callback() and log_callback:
                    log_callback("[*] Requested another xAI verification email")
            except Exception as exc:
                if log_callback:
                    log_callback(f"[Debug] xAI resend failed: {type(exc).__name__}")
            next_resend_at = time.time() + 45
        try:
        ...
        for message in messages:
            ...
            for item in recipients:
                if isinstance(item, dict):
                    recipient_values.append(str(item.get("address") or item.get("email") or "").lower())
                else:
                    recipient_values.append(str(item).lower())
...
def _legacy_generate_random_birthdate():
...
def _legacy_set_birth_date(session, log_callback=None):
    ...
    payload = {"birthDate": _legacy_generate_random_birthdate()}
    try:
...
def _legacy_set_tos_accepted(session, log_callback=None):
...
def _legacy_encode_grpc_nsfw_settings():
...
def _legacy_update_nsfw_settings(session, log_callback=None):
    url = "https://grok.com/auth_mgmt.AuthManagement/UpdateUserFeatureControls"
    data = _legacy_encode_grpc_nsfw_settings()
    new_headers = {
        "content-type": "application/grpc-web+proto",
        "x-grpc-web": "1",
        "origin": "https://grok.com",
...
def enable_nsfw_for_token(token, cf_clearance="", log_callback=None):
    ...
    try:
        with requests.Session(impersonate="chrome120", proxies=proxies) as session:
            ...
            if not _legacy_set_tos_accepted(session, log_callback):
                return False, "set_tos_accepted 失败!"
            if not _legacy_set_birth_date(session, log_callback):
                return False, "set_birth_date 失败!"
            if not _legacy_update_nsfw_settings(session, log_callback):
...
from tab_pool import TabPool  # noqa: E402
...
def fill_email_and_submit(timeout=15, log_callback=None, cancel_callback=None, email_provider=None):
    page = _get_page()
    raise_if_cancelled(cancel_callback)
    check_timeout(time.time())
    email, dev_token = get_email_and_token(provider=email_provider)
...
return true;
        ...
        if clicked:
            ...
            confirm_timeout = max(3, int(config.get("email_submit_confirm_timeout", 30) or 30))
...
if (codeInput) return { ready: true, error: '' };
const errorNode = Array.from(document.querySelectorAll(
  '[role="alert"], [data-testid*="error"], .text-red-500, .text-destructive'
)).find((node) => visible(node) && (node.innerText || node.textContent || '').trim());
return {
  ready: false,
  error: errorNode ? (errorNode.innerText || errorNode.textContent || '').trim().slice(0, 240) : ''
};
                    """
                ) or {}
                if state.get("ready"):
                    if log_callback:
                        log_callback("[*] xAI verification form is ready")
                    return email, dev_token
                if state.get("error"):
                    raise Exception(f"email_submit_rejected: {state['error']}")
                human_sleep(0.5, cancel_callback)
            raise Exception("email_submit_not_confirmed")
        human_sleep(0.5, cancel_callback)
    raise Exception("未找到邮箱输入框或注册按钮")


def fill_code_and_submit(email, dev_token, timeout=180, log_callback=None, cancel_callback=None, email_provider=None):
    page = _get_page()
    check_timeout(time.time())
...
return false;
    ...
    mail_timeout = max(15, int(config.get("mail_timeout", timeout) or timeout))
    mail_poll_interval = max(0.5, float(config.get("mail_poll_interval", 3) or 3))
    code = get_oai_code(
        dev_token,
        email,
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        self.email_provider_combo = ttk.Combobox(config_frame, textvariable=self.email_provider_var, values=["duckmail", "yyds", "tempmail_lol", "cloudflare", "cloudmail"], width=12, state="readonly")
        ...
        self.oauth_btn = ttk.Button(btn_frame, text="生成OAuth", command=self.start_oauth_batch)
        self.oauth_btn.pack(side=tk.LEFT, padx=5)
...
8) 代理（可选）
- 不填=直连
- 示例: http://proxy-host:8118（服务器部署时必须从容器内可达）
- 代理不稳会影响验证码和注册稳定性
...
- 如果关闭了启动教程，可随时点主界面的“教程”按钮重新打开。"""
    ...
    def start_oauth_batch(self):
        if self.is_running:
            self.log("[!] 当前已有任务在运行")
            return

        accounts_file = os.path.join(os.path.dirname(__file__), "accounts_cli.txt")
        ...
        selected = filedialog.askopenfilename(
            ...
            initialfile="accounts_cli.txt"
        )
        if not selected:
            self.log("[*] 已取消")
            return
        accounts_file = selected
        self.log(f"[*] 使用账号文件: {accounts_file}")

        if not os.path.exists(accounts_file):
            self.log(f"[!] 账号文件不存在: {accounts_file}")
            return

        self.log("[*] 开始批量生成OAuth...")
        self._set_running_ui(True)
        self.stop_requested = False
        threading.Thread(
            target=self.run_oauth_batch,
            args=(accounts_file,),
            daemon=True,
        ).start()

    def run_oauth_batch(self, accounts_file):
        try:
            import subprocess
            script_path = os.path.join(os.path.dirname(__file__), "scripts", "backfill_cpa_xai_from_accounts.py")
            result = subprocess.run(
                [sys.executable, "-u", script_path, "--accounts", accounts_file, "--limit", "0", "--probe"],
                cwd=os.path.dirname(__file__),
                env={**os.environ, "DISPLAY": os.environ.get("DISPLAY", ""), "PYTHONIOENCODING": "utf-8"},
                capture_output=True,
                text=True,
                encoding="utf-8",
                errors="replace",
            ...
            if result.returncode == 0:
                self.log("[*] OAuth批量生成完成")
            else:
                self.log(f"[!] OAuth批量生成失败: {result.returncode}")
        except Exception as e:
            self.log(f"[!] 运行错误: {e}")
        finally:
...
def main():
    if tk is None:
        raise RuntimeError("Desktop GUI requires tkinter; use register_cli.py on servers")
    root = tk.Tk()
