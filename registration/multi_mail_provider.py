...
DATA_DIR = Path(os.environ.get("REGISTRATION_DATA_DIR") or Path(__file__).resolve().parent)


def read_json_file(path: Path, *, default_factory, expected_types):
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        return default_factory()
    return value if isinstance(value, expected_types) else default_factory()
...
class TempMailLolProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        self.api_base = str(entry.get("api_base") or "https://api.tempmail.lol").rstrip("/")
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)):
        resp = self.session.request(method.upper(), f"{self.api_base}/v2{path}", params=params, json=payload, timeout=self.conf["request_timeout"], verify=False)
        if resp.status_code not in expected:
...
class YydsMailProvider(BaseMailProvider):
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        # YYDSMail rejects `subdomain` on the regular endpoint without a
        # matching `domain`; the wildcard endpoint is the valid form for a
        # subdomain-only configuration.
        use_wildcard_endpoint = self.wildcard or bool(self.subdomain and not self.domain)
        data = self._request("POST", "/accounts/wildcard" if use_wildcard_endpoint else "/accounts", payload=payload)
...
def _create_provider(mail_config: dict, provider: str = "", provider_ref: str = "") -> BaseMailProvider:
    ...
    if entry["type"] in {"yyds_mail", "yyds"}:
