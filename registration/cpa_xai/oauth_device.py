    ...
    *,
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            log(f"OAuth 轮询网络波动（{net_streak}/{max_net_streak}）：{e}，{wait} 秒后重试")
        ...
        if err in ("authorization_pending", "slow_down"):
            ...
            state_text = "等待用户授权" if err == "authorization_pending" else "服务端要求降低轮询速度"
            log(f"OAuth {state_text}，{sleep_for} 秒后重试")
        ...
        if status >= 500 or status in (502, 503, 504) or not isinstance(body, dict):
            ...
            log(f"OAuth 轮询收到临时 HTTP {status}，{wait} 秒后重试")
            if net_streak >= max_net_streak:
                raise OAuthDeviceError(
            ...
            time.sleep(wait)
            continue
        log(f"OAuth 轮询收到非预期 HTTP {status}，稍后重试")
        time.sleep(sleep_for)
