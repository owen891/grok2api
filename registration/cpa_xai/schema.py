...
def normalize_cpa_base_url(value: str = DEFAULT_BASE_URL) -> str:
    base_url = (value or DEFAULT_BASE_URL).strip().rstrip("/")
    parsed = urlparse(base_url)
    host = (parsed.hostname or "").lower()
    allowed = host in {"x.ai", "grok.com"} or host.endswith((".x.ai", ".grok.com"))
    if parsed.scheme != "https" or not allowed or parsed.username or parsed.password:
        raise ValueError("CPA base_url must be an allowed x.ai/grok.com HTTPS URL")
...
def build_cpa_xai_auth(
    *,
    ...
    # CPA joins baseURL + "/responses" and must not send credentials off-domain.
