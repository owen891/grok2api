...
class XConsoleAuthClient:
    ...
    def _scrape_action_id(self, html: str) -> str:
        ...
        if not ordered:
            raise RuntimeError(
                "No JavaScript chunks found in the x.ai sign-up page. "
                "The response may be a Cloudflare challenge, an access-denied "
                "page, or an upstream deployment error."
            )
    ...
    def _grpc_call(self, url: str, fields: List[Tuple[int, str]], referer: str) -> GrpcResult:
        ...
        status, resp_headers, _, raw = self._request(
            "POST", url, headers=headers, body=body
        )
        # Most gRPC-web responses include a trailer frame. Some edge errors use
        # an empty body and expose grpc-status/grpc-message as HTTP headers.
        if not raw:
            grpc_status = None
            try:
                if "grpc-status" in resp_headers:
                    grpc_status = int(resp_headers["grpc-status"])
            except (TypeError, ValueError):
                grpc_status = None
            trailers = {}
            if grpc_status is not None:
                trailers["grpc-status"] = str(grpc_status)
            if resp_headers.get("grpc-message"):
                trailers["grpc-message"] = unquote(resp_headers["grpc-message"])
            return GrpcResult(
                ok=(status == 200 and grpc_status == 0),
                http_status=status,
