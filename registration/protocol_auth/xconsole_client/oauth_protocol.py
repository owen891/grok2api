...
def extract_submit_oauth_action(source: str) -> str:
    """Extract the server action nearest to submitOAuth2Consent."""
    if not source or "submitOAuth2Consent" not in source:
        return ""
    direct_patterns = (
        r'createServerReference\)\(["\']([a-f0-9]{40,64})["\'][^;]{0,1200}?["\']submitOAuth2Consent["\']',
        r'createServerReference\(["\']([a-f0-9]{40,64})["\'][^;]{0,1200}?["\']submitOAuth2Consent["\']',
        r'["\']submitOAuth2Consent["\'][^;]{0,1200}?createServerReference\)\(["\']([a-f0-9]{40,64})["\']',
    )
    for pattern in direct_patterns:
        match = re.search(pattern, source, re.IGNORECASE)
        if match:
            return match.group(1)
    hashes = re.compile(r"[a-f0-9]{40,64}", re.IGNORECASE)
    for marker in re.finditer(r"submitOAuth2Consent", source):
        start = max(0, marker.start() - 800)
        end = min(len(source), marker.end() + 800)
        window = source[start:end]
        candidates = list(hashes.finditer(window))
        if not candidates:
            continue
        marker_offset = marker.start() - start
        nearest = min(candidates, key=lambda item: abs(item.start() - marker_offset))
        return nearest.group(0)
    return ""


def script_sources(page_url: str, page_html: str) -> List[str]:
    ...
    # Next.js also serializes route-specific chunks inside RSC flight data. Those
    # chunks are not always emitted as ordinary script tags on the consent page.
    for match in re.finditer(r'''/_next/static/chunks/[^"'\s]+?\.js''', page_html or ""):
        sources.append(match.group(0))
    return list(dict.fromkeys(urljoin(page_url, value) for value in sources))
    ...
    def login(
        ...
        *,
        ...
        def _submit_oauth2_consent(page_url: str, page_html: str = "") -> str:
            ...
            action_id = extract_submit_oauth_action(page_html)
            if not action_id:
                for source_url in script_sources(page_url, page_html)[:96]:
                    try:
            ...
            if not action_id:
                ...
                self._log("live submitOAuth2Consent action not found; using bundled fallback")
