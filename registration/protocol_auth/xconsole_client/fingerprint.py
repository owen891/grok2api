...
The default impersonate target is `chrome`, which lets the installed
`curl_cffi` select its current Chrome profile. Pinning the older `chrome131`
profile while sending newer visible browser headers is rejected by the current
x.ai Cloudflare edge.
...
def _retry_setting(name: str, default: float, *, minimum: float, maximum: float) -> float:
    try:
...
def _is_retryable_connection_error(exc: Exception) -> bool:
    """Retry failures that happen before an HTTP request can be established."""
    text = str(exc).lower()
    return any(
        marker in text
        for marker in (
...
class FingerprintTransport:
    ...
    def __init__(
        ...
        *,
        ...
        retry_count = _retry_setting(
            "GROK2API_XCONSOLE_HTTP_RETRIES",
            DEFAULT_MAX_RETRIES if max_retries is None else max_retries,
            minimum=0,
            maximum=5,
        )
        self._max_retries = int(retry_count)
        self._retry_backoff = _retry_setting(
            "GROK2API_XCONSOLE_RETRY_BACKOFF",
            DEFAULT_RETRY_BACKOFF if retry_backoff is None else retry_backoff,
            minimum=0.0,
            maximum=10.0,
        )
            ...
            try:
                ...
                if attempt >= attempts or not _is_retryable_connection_error(exc):
                    if _is_retryable_connection_error(exc):
                        route = "proxy" if self._proxy else "direct"
                        raise RuntimeError(
                            f"x.ai connection failed for {url} via {route} route "
                ...
                if self._debug:
                    print(
                        f"  [retry] {method} {url} connection failed "
