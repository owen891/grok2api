...
const endpoints: Record<string, EndpointDefinition> = {
  ...
  "image/generations": {
    ...
    fields: [
      ...
      { name: "async", descriptionKey: "docs.reference.fieldImageAsync" },
    ],
    noteKeys: ["docs.reference.noteImageCount", "docs.reference.noteImageStorage", "docs.reference.noteImageAsync"],
    request: (model) => ({ model, prompt: "A minimal red chair in a bright studio", n: 1, response_format: "url", async: true }),
    response: { request_id: "image_example" },
  },
  "image/get": {
    key: "image/get", category: "Image", title: "Get image job", method: "GET", path: "/images/{request_id}",
    descriptionKey: "docs.endpointImageGet", capabilities: ["image"],
    fields: [{ name: "request_id", required: true, descriptionKey: "docs.reference.fieldRequestId" }],
    noteKeys: ["docs.reference.noteImagePolling"],
    request: () => undefined,
    response: { status: "done", model: "grok-imagine-image", progress: 100, created: 1783860000, data: [{ url: "http://127.0.0.1:8000/v1/media/images/example" }] },
  },
  "image/edits": {
