...
- 将公开模型别名替换为上游模型 ID。
- 显式 `prompt_cache_key` 优先作为会话种子；未提供时可读取 Claude Code 的 `X-Claude-Code-Session-Id`、`metadata.session_id` 或 `metadata.user_id` 内嵌/后缀会话标识。Build 路由会结合客户端密钥、上游模型和调用协议生成固定长度的隔离 identity，同时用于账号粘滞、CLI 会话 Header 与上游请求体，避免共享账号池中的跨租户碰撞。
...
- `egress_mode`（`proxy` 或 `direct`）
- `egress_scope`
- `egress_node_id` / `egress_node_name`（出口节点快照）

不保存 prompt、response body、encrypted reasoning 内容、工具参数、代理 URL、代理认证信息、User-Agent 或 Cookie。
...
- 出口节点按 `Grok Build`、`Grok Web`、`Grok Console`、`Grok Web（仅资源）` 四个作用域独立管理代理、健康度和冷却状态。Grok Build 始终沿用 Provider 的 CLI User-Agent，节点不能覆盖；Grok Web、Console 和 Web 资源节点独立管理浏览器 User-Agent 与 Cloudflare Cookie。Web 资源未配置专用节点时仅回退到 Grok Web 节点。
