...
func (a *Adapter) GenerateVideo(ctx context.Context, request provider.VideoRequest) (provider.VideoResult, error) {
	...
	// Video fallback is gated by the same persisted entitlement marker as the
	// Responses fallback. Billing may be absent or stale in this async path.
	if !xaiEligible && !a.canUseBuildAPIFallback(ctx, credential.ID) {
		return provider.VideoResult{}, createErr
	}
